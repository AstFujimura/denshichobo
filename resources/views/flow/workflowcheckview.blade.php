            ...
            <div class="flow_view_content">
                <div class="flow_view_table">
                    ...
                    <div class="ongoing_tab {{($status == 'ongoing_tab' ? 'open_tab' : '')}}">
                        ...
                        <div class="none_data_message">進行中の閲覧データはありません</div>
                        @endif
                        @foreach ($t_flows_ongoing as $t_flow_ongoing)
                        <a href="{{route('workflowcheckdetailget', $t_flow_ongoing->flow_id)}}" class="flow_view_tbody_tr">
                    ...
                    <div class="approved_tab {{($status == 'approved_tab' ? 'open_tab' : '')}}">
                        ...
                        <div class="none_data_message">決裁済の閲覧データはありません</div>
                        @endif
                        @foreach ($t_flows_approved as $t_flow_approved)
                        <a href="{{route('workflowcheckdetailget', $t_flow_approved->flow_id)}}" class="flow_view_tbody_tr">
                            <div class="flow_view_td flow_view_title">
                                {{$t_flow_approved->標題}}
                            </div>
                            <div class="flow_view_td flow_view_category">
                    ...
                    <div class="reject_tab  {{($status == 'reject_tab' ? 'open_tab' : '')}}">
                        @if ($t_flows_reject->count() == 0)
                        <div class="none_data_message">却下済の閲覧データはありません</div>
                        @endif
                        @foreach ($t_flows_reject as $t_flow_reject)
                        <a href="{{route('workflowcheckdetailget', $t_flow_reject->flow_id)}}" class="flow_view_tbody_tr">
                            <div class="flow_view_td flow_view_title">
                                {{$t_flow_reject->標題}}
                            </div>
                            <div class="flow_view_td flow_view_category">
                    ...
                    <div class="reapplication_tab {{($status == 'reapplication_tab' ? 'open_tab' : '')}}">
                        ...
                        <div class="none_data_message">再申請待ちの閲覧データはありません</div>
                        @endif
                        @foreach ($t_flows_reapplication as $t_flow_reapplication)
                        <a href="{{route('workflowcheckdetailget', $t_flow_reapplication->flow_id)}}" class="flow_view_tbody_tr">
                            <div class="flow_view_td flow_view_title">
